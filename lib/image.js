...
class Image {
  ...
  static async palette(url) {
    ...
    palette = _.mapValues(palette, swatch =>
      _.mapKeys(swatch, (value, key) => key.slice(1))
    );
  ...
  // TODO: replace deprecated resize methods http://sharp.pixelplumbing.com/en/stable/changelog/#v0210-4th-october-2018
  ...
  static async transform(url, settings) {
    ...
    if (settings.w || settings.h) {
      ...
      if (settings.w && settings.h && settings.g) {
        ...
        if (
          /^(north|northeast|east|southeast|south|southwest|west|northwest|center|centre)$/.test(
            g
          )
        ) {
          position = sharp.gravity[g];
        }
        if (/^(entropy|attention)$/.test(g)) {
          position = sharp.strategy[g];
        }
      }
      ...
      if (settings.b) {
        ...
        background = settings.b;
      } else if (!/(png|webp|svg)/.test(settings.outputFormat)) {
        background = 'white';
      }

      image.resize({
        width: parseInt(settings.w, 10) || null,
        height: parseInt(settings.h, 10) || null,
        fit,
        position,
        background,
      });
