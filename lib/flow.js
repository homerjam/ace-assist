...
class Flow {
  ...
  checkChunk(chunkNumber = 0, chunkSize = 0, totalChunks = 0, totalSize = 0, identifier = '', filename = '') {
    chunkNumber = typeof chunkNumber !== 'number' ? parseInt(chunkNumber, 10) : chunkNumber;
    chunkSize = typeof chunkSize !== 'number' ? parseInt(chunkSize, 10) : chunkSize;
    totalSize = typeof totalSize !== 'number' ? parseInt(totalSize, 10) : totalSize;
      ...
      fs.statAsync(chunkFilename)
        .then(() => {
          if (chunkNumber === totalChunks) {
            this.finaliseChunks(totalChunks, identifier, filename)
              .then(resolve, reject);
            return;
          }
          resolve();
  ...
  finaliseChunks(totalChunks, identifier, filename, originalFilename) {
    return new Promise((resolve, reject) => {
      this.mergeChunks(totalChunks, identifier, filename)
        .then(() => {
