...
class Flow {
  ...
  validate(chunkNumber, chunkSize, totalSize, identifier, filename, fileSize) {
    chunkNumber =
      typeof chunkNumber !== 'number' ? parseInt(chunkNumber, 10) : chunkNumber;
    chunkSize =
      typeof chunkSize !== 'number' ? parseInt(chunkSize, 10) : chunkSize;
    totalSize =
      typeof totalSize !== 'number' ? parseInt(totalSize, 10) : totalSize;
    ...
    if (typeof fileSize !== 'undefined') {
      fileSize =
        typeof fileSize !== 'number' ? parseInt(fileSize, 10) : fileSize;
  ...
  checkChunk(
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.statAsync(chunkFilename).then(() => {
        if (chunkNumber === totalChunks) {
          this.finaliseChunks(totalChunks, identifier, filename).then(
            resolve,
            reject
          );
          return;
  ...
  mergeChunks(numberOfChunks, identifier, filename) {
    return new Promise((resolve, reject) => {
      ...
      Promise.all(chunks).then(results => {
        fs.writeFileAsync(
          path.join(this.temporaryFolder, filename),
  ...
  finaliseChunks(totalChunks, identifier, filename, originalFilename) {
    return new Promise((resolve, reject) => {
      this.mergeChunks(totalChunks, identifier, filename).then(() => {
        this.removeChunks(identifier).then(() => {
          resolve({
            status: 'complete',
            filename,
            originalFilename,
  ...
  saveChunk(
    ...
    chunkSize =
      typeof chunkSize !== 'number' ? parseInt(chunkSize, 10) : chunkSize;
    totalChunks =
      typeof totalChunks !== 'number' ? parseInt(totalChunks, 10) : totalChunks;
    totalSize =
      typeof totalSize !== 'number' ? parseInt(totalSize, 10) : totalSize;

    return new Promise((resolve, reject) => {
      if (
        !files[this.fileParameterName] ||
        !files[this.fileParameterName].size
      ) {
      ...
      if (validation === 'valid') {
        ...
        mv(files[this.fileParameterName].path, chunkFilename, error => {
          if (error) {
            console.error(error);
          }
          ...
          const testChunkExists = testChunk => {
            fs.stat(this.getChunkFilename(testChunk, identifier), error => {
              if (!error) {
                if (testChunk === totalChunks && chunkNumber === totalChunks) {
                  this.finaliseChunks(totalChunks, identifier, filename).then(
                    resolve,
                    reject
                  );
        ...
        reject(new Error(`${validation}: ${filename} ${identifier}`));
      }
    });
  }
