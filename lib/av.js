...
module.exports = class AV {
  ...
  static async process(filePath) {
    ...
    if (videoStream) {
      if (!_.isNumber(metadata.format.duration) && metadata.format.format_name !== 'gif') {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const promise = new Promise(async (resolve, reject) => {
        ...
        if (/https?:/.test(filePathOrUrl)) {
          ...
          input = (await axios.get(filePathOrUrl, { responseType: 'arraybuffer' })).data;
        } else {
          input = await fs.readFileAsync(filePathOrUrl);
        }

        if (input instanceof Buffer) {
          inputFile = path.join(__dirname, '../tmp', `${hashKey}.in.${settings.inputFormat}`);
          ...
          .on('end', async (stdout, stderr) => {
            try {
              ...
              if (buffer.length < 1024) {
                const error = new Error('Conversion failed: result too small / input stream failure');
